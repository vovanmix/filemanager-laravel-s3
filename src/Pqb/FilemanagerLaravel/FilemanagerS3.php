...
class FilemanagerS3 extends Filemanager {
	 ...
	 * @var string
	 */
	protected $folderIcon       = '/vendor/Filemanager/images/fileicons/_Open.png';
	/**
	 * root directory
	 *
	 * @var string
...
//			 'Body' => \Guzzle\Http\EntityBody::factory(fopen($_FILES['newfile']['tmp_name'], 'r'))
		 ));
...
//			$this->s3->get_object_list($this->bucket, array('prefix' => $Path))
		...
		$Error    = !empty($response) ? '' : sprintf($this->lang('INVALID_DIRECTORY_OR_FILE'), $Path);
		$Code     = !empty($response) ? 0  : -1;
...
//		$objects = $this->s3->get_object_list($this->bucket, array('prefix' => $directory));
		...
		$objects = array_filter($objects['Contents'], function($filePath) use($directory){
			return trim($directory, '/ ') !== trim($filePath['Key'], '/ ');
		});
		$_this   = $this;
		$objects = array_filter($objects, function($filePath) use($_this, $directory){
			$filePath = preg_replace("@^{$directory}@", '', $filePath['Key']);
		...
		foreach ($objects as $filePath) {
			...
			$fileList[$filePath['Key']] = $this->fileInfo($filePath['Key']);
		}
		return $fileList;
	}
	/**
	...
	public function getinfo() {
		$fileInfo = $this->fileInfo($this->buildFullPath());
        $fileInfo['Path'] = $fileInfo['Preview'];
		return $fileInfo;
	}
...
//			$this->s3->get_object_list($this->bucket, array('prefix' => $oldPath))
		...
		if ($isDir) {
			...
			$response = $this->s3->copyObject([
                'Bucket' => $this->bucket,
                'CopySource' => urlencode($this->bucket.'/'.$oldPath),
                'Key' => $newPath
            ]);
...
//			);
	...
	protected function fileInfo($filePath) {
		...
		$fileInfo['Preview']     = ($isDirectory) ? $this->folderIcon   : sprintf('//%s/%s', $this->domain, $this->bucket.'/'.$filePath);
...
//		);
     ...
     * @return bool
     */
	protected function isUploadValidSize($fileName = 'newfile') {
		$sizeInBytes = $_FILES[$fileName]['size'];
...
//			$this->s3->get_object_list($this->bucket, compact('prefix'));
